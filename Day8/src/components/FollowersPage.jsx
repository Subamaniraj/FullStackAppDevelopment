...
class FollowersPage extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      search: '', // Initialize search state
    };
  }
  ...
  renderFollowers() {
    const filteredFollowers = this.state.followers.filter((follower) =>
      follower.name.toLowerCase().includes(this.state.search.toLowerCase())
    );

    return filteredFollowers.map((follower) => (
